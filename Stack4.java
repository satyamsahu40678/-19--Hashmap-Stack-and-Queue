...
public class Stack4 {
        ...
        for(int i=1; i<arr.length; i++){
            ...
            while(j>0 && (list.get(j)<0 && list.get(j-1)>=0)){
                ...
                if(Math.abs(first)==Math.abs(second)){
                    //both have same value then both will destory
                    j=j-2;
                    continue;
                }else if(Math.abs(first)>Math.abs(second)){
                    //first have greater value then 2nd will destory
                    j--;
                    list.add(first);
                }else{
                    //2nd have greater value then 1st will destory
                    j--;
                    list.add(second);
                }
            }
    ...
    public static void main(String[] args) {
        ...
        int []arr2 = asteroidCollision(arr1);
